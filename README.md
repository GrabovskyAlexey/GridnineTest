...
### Реализация
Интерфейс Rule является базовым для создания правил фильтрации. В методе `check(Flight flight)` описывается логика фильтрации.
Реализовал следующие правила фильтрации:
* AlreadyDepartedExcludeRule - Исключает перелеты с сегментами в которых время вылета меньше текущего момента
* ArrivalBeforeDepartureExcludeRule - Исключает перелеты с сегментами в которых время прилета меньше времени вылета
* HoursGroundTimeExcludeRule - Исключает перелеты с сегментами в которых перерыв между сегментами превышает заданное в конструкторе количество часов

Интерфейс Filter и его реализация FilterImpl предоставляет методы для добавления новых правил фильтрации, очистки списка правил и фильтрации данных